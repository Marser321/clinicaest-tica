...
export function TestimonialsSection() {
    return (
        <section className="section-padding relative overflow-hidden bg-[var(--color-bg-secondary)]">
            {/* Decoración */}
                ...
                <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                    {testimonios.map((test, i) => (
                        <motion.div
                            ...
                            whileHover={{ y: -5 }}
                            className="bg-[var(--color-bg-primary)]/40 backdrop-blur-md rounded-2xl p-6 sm:p-7 flex flex-col relative border border-[var(--color-glass-border)] shadow-[0_4px_24px_rgba(0,0,0,0.5)] hover:border-[var(--color-accent)]/30 transition-all duration-400 group"
                        >
                            {/* Header: Avatar + Quote */}
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-12 h-12 rounded-full bg-red-950/40 flex items-center justify-center border border-red-900/30 group-hover:scale-110 transition-transform duration-300">
                                    <span className="font-display font-bold text-[var(--color-accent)] text-lg">
                                        {test.nombre.charAt(0)}
                                    </span>
                                </div>
                                <Quote size={24} className="opacity-20 group-hover:opacity-50 group-hover:text-[var(--color-accent)] transition-colors" />
                            </div>
                            ...
                            {/* Texto */}
                            <p className="text-[var(--color-text-secondary)] text-sm leading-relaxed flex-1 mb-5 relative z-10">
                                &ldquo;{test.texto}&rdquo;
                            </p>

                            {/* Autor */}
                            <div className="pt-4 border-t border-[var(--color-glass-border)]">
                                <p className="font-semibold text-[var(--color-text-primary)] text-sm">{test.nombre}</p>
                                <p className="text-[var(--color-text-muted)] text-xs">{test.tratamiento}</p>
                            </div>
                        </motion.div>
                    ))}
                </StaggerContainer>
